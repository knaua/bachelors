...
use crate::booking_process::{end_booking, start_booking, wireguard_add_peer};
use crate::db_manager::{add_interface_to_db, add_device_to_db, Db, retrieve_interface_data};
...
pub struct DeviceInterfaceData {
    public_key: String,
    wg_ip_address: String, // This needs to be the IP-Address without a prefix
    interface_id: String,
}
...
// TODO This should be extended with some sort of key that is only known to the team and the server,
...
async fn add_device_to_interface(data: Json<DeviceInterfaceData>) {
    let device = data.0;
    let _ = wireguard_add_peer(&device.interface_id, &device.public_key, device.wg_ip_address);
}

#[post("/get_interface_data", format = "json", data = "<data>")]
async fn get_interface_data(data: Json<DeviceInterfaceData>, mut db: Connection<Db>) -> String {
    retrieve_interface_data(data.0, &mut db).await
...
// The "secrets" feature also needs to be enabled for rocket in the Cargo.toml
#[get("/")]
fn _index() -> Template {
...
async fn rocket() -> _ {
    rocket::build()
        ...
        .mount("/", routes![reserve, add_device, add_interface, end_reservation, add_device_to_interface, get_interface_data])
        //.mount("/login", cred::routes()) // Mount the routes for the login mechanism
}
